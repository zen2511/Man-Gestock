  ...
  return (
    <div style={S.section}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        ...
        <button onClick={resetDefaut} style={S.btnReset}>Réinitialiser</button>
      </div>
      ...
      <div style={S.addRow}>
        ...
        />
        <button onClick={ajouter} style={S.btnAjouter}>Ajouter</button>
      </div>
    </div>
  )
}
        ...
        {onglet === 'utilisateurs' && userActif?.role === 'admin' && (
          <div style={S.section}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              ...
              <button onClick={ouvrirNouveau} style={S.btnPrimary}>+ Ajouter</button>
            </div>
                    ...
                    <tr key={u.id} style={S.tr}>
                      <td style={S.td}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: 10, color: '#94a3b8' }}>{u.dateCreation?.slice(0, 10)}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td style={S.td}>
                        <div style={{ display: 'flex', gap: 6 }}>
                          ...
                          <button onClick={() => suppUser(u.id)} style={S.btnDel} disabled={u.id === userActif?.id}>Retirer</button>
                        </div>
                      </td>
                    </tr>
                  ))}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {Object.entries(d.peut).map(([perm, val]) => (
                      <div key={perm} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#475569' }}>
                        <span>{perm}</span>
                        <span style={{ fontWeight: 700, color: val ? B[600] : '#94a3b8' }}>{val ? 'Oui' : 'Non'}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div style={S.modalOverlay}>
          <div style={S.modal}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <button onClick={() => setModalUser(null)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#94a3b8' }}>×</button>
            </div>
