                  ...
                  {prodsCat.map(p => {
                    ...
                    return (
                      <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0', borderBottom: `1px solid ${B[50]}`, fontSize: 12 }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ color: '#334155', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 500 }}>
                            {p.designation || p.nom || '—'}
                          </div>
                          {p.reference && (
                            <div style={{ fontSize: 10, color: '#94a3b8', fontFamily: 'monospace' }}>{p.reference}</div>
                          )}
                        </div>
                        <span style={{
